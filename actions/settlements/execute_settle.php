<?php
// actions/settlements/execute_settle.php
// Supplier EOD Settlement & Loan Recovery Action Controller
...
$defaultRate          = (float)getSetting('default_commission_rate', 6.00);
$commissionRate       = floatval($_POST['commission_rate'] ?? $defaultRate);
...
try {
    ...
    // Deduct from supplier loan balance & record loan repayment log
    if ($loanDeduction > 0) {
        ...
        $stmtLoanLog = $db->prepare("
            INSERT INTO supplier_loans (supplier_id, amount, type, notes, issued_by, created_at)
            VALUES (:sid, :amt, 'repayment', :notes, :uid, NOW())
        ");
        $stmtLoanLog->execute([
            ':sid'   => $consignment['supplier_id'],
            ':amt'   => $loanDeduction,
            ':notes' => "EOD Auto Deduction for Settlement {$settlementNo}",
            ':uid'   => currentUser()['id']
        ]);
    }
