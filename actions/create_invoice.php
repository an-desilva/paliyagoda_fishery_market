<?php
// actions/create_invoice.php
// Transactional POS Sale, Invoice Generation, Security Hash, Credit Limit Check & Gate Pass
...
$itemsRaw    = $_POST['items'] ?? []; // Array of ['fish_id' => int, 'unit_price' => float]
...
try {
    ...
    if ($buyer['status'] === 'blocked') {
        throw new Exception("Buyer '{$buyer['name']}' account is BLOCKED due to unpaid credit ledger.");
    }
    ...
    if ($paymentMode === 'credit') {
        ...
        if ($newCreditBalance > $buyer['credit_limit']) {
            ...
            throw new Exception("Credit Limit Exceeded! Buyer '{$buyer['name']}' limit is " . formatLKR($buyer['credit_limit']) . ". Current Balance: " . formatLKR($buyer['current_credit_balance']) . ". Bill Total: " . formatLKR($grandTotal) . ". Over by {$exceeded}.");
        }
    }
    ...
    // 5. Insert Invoice Items & Update Fish Item Status
    $stmtItem = $db->prepare("
        INSERT INTO invoice_items (invoice_id, fish_item_id, weight_kg, unit_price, line_total)
        VALUES (:inv_id, :fish_id, :w, :u, :lt)
    ");
    ...
    setFlash('success', "Invoice {$invoiceNo} Created Successfully! Gate Pass Verification Code: {$securityHash}");
